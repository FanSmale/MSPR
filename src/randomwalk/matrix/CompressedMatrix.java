...
public class CompressedMatrix {
    ...
    */
	 ...
	 *********************
	 */
	public CompressedMatrix(String paraFilename, int[][][] paraNewIndex, int paraAttribute) {
		data = null;
		try {
			FileReader fileReader = new FileReader(paraFilename);
			data = new Instances(fileReader);
		...
		Instance tempLastInstance = data.instance(data.numInstances() - 1);
		// int tempNumNodes = (int) tempLastInstance.value(0) + 1;
		int tempNumNodes = paraNewIndex[paraAttribute].length;
		System.out.println("There are " + tempNumNodes + " nodes.");
		matrix = new Triple[tempNumNodes];
		Triple[] tails = new Triple[tempNumNodes];
		...
		for (int i = 0; i < data.numInstances(); i++) {
			...
			int tempOldRow = (int)data.instance(i).value(0);
			int tempOldColumn = (int)data.instance(i).value(1);
			
			int tempRow = 0;
			int tempColumn = 0;
			
			for(int j = 0; j < paraNewIndex[i].length; j++){
				if(paraNewIndex[i][j][1] == tempOldRow){
					tempRow = paraNewIndex[i][j][0];
				}
				if(paraNewIndex[i][j][1] == tempOldColumn){
					tempColumn = paraNewIndex[i][j][0];
				}
			}// end of for j
	 ...
	 *********************
	 */
	public CompressedMatrix(String paraFilename, int paraNumNodes) {
		data = null;
		try {
			FileReader fileReader = new FileReader(paraFilename);
			data = new Instances(fileReader);
	...
	*/
	...
	public static void main(String args[]) {
		...
		CompressedMatrix[] ee= new CompressedMatrix[4];
		
		System.out.println("Testing CompressedMatrix!");
		CompressedMatrix tempMatrix = new CompressedMatrix("D:/Java/randomwalk/data/graph.arff");
